...
class VLMDoubleCritic(torch.nn.Module):
    def __init__(self, device, accelerator, critic_lm, cache_dir, in_dim, out_dim):
        ...
        out_dim = 2
        self.v_critic1 = nn.Sequential(nn.Linear(in_dim+image_feature_dim, in_dim),\
                                    nn.ReLU(),\
                                    nn.Linear(in_dim, in_dim),\
                                    nn.ReLU(),\
                                    nn.Linear(in_dim, out_dim)).to(device)
        self.v_critic2 = nn.Sequential(nn.Linear(in_dim+image_feature_dim, in_dim),\
                                    nn.ReLU(),\
                                    nn.Linear(in_dim, in_dim),\
                                    nn.ReLU(),\
                                    nn.Linear(in_dim, out_dim)).to(device)
    ...
    def forward(self, observation, image_features, action, detach_model=False):
        ...
        return self.v_critic1(v_states), self.v_critic2(v_states)
...
class TrajectoryCritic(torch.nn.Module):
    def __init__(self, device, accelerator, critic_lm, cache_dir, in_dim, out_dim):
        ...
        self.device = device
        self.accelerator = accelerator
        self.base_lm = AutoModel.from_pretrained(critic_lm, cache_dir=cache_dir).to(device).to(dtype=torch.bfloat16)
        self.dtype = self.base_lm.dtype
        self.base_tokenizer = AutoTokenizer.from_pretrained(critic_lm, cache_dir=cache_dir)
        self.base_tokenizer.truncation_side = 'left'
        out_dim = 2
        self.critic = nn.Linear(in_dim, out_dim).to(device).to(dtype=torch.bfloat16)
