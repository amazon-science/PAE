...
class DummyImageDataset(Dataset):
    def __init__(self, buffer, use_str=False):
        self.buffer = buffer
    ...
    def __getitem__(self, idx):
        obs_copy = deepcopy(self.buffer[idx])
        if not self.use_str:
            obs_copy["observation"]["image"] = np.array(Image.open(obs_copy["observation"]["image"]).convert("RGB"))
        # obs_copy["action"] = "Action: " + obs_copy["action"].split("Action: ")[1]
        return obs_copy
